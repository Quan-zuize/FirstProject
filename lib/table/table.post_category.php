...
function post_categoryDelete($category_id) 
{
			...
			db_q("DELETE FROM `post_to_post_category` WHERE `category_id` = {$category_id}");
 ...
 */
function post_categoryGetTotal()
{
		
		$sql = "SELECT COUNT(*) FROM `post_category`";
...
function post_categoryFeatureds()
{
	...
	foreach ($rs as $post_category)
	{
		...
		$post_categories[] = array(
				...
				'href'        => "/post-post_category.php?path=" . $post_category['category_id'],
				'width'       => 370,
				'height'      => 240,
		);
	}
